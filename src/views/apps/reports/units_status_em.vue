  ...
  setup(props) {
    ...
    const items = ref<Array<string>>([]);
     ...
     .then(({ data }) => {
      ...
      {
        ApiService.get(`units_status/emergency?page=${currentPage.value}&sortDesc=${sortDesc.value}`).then(({ data }) =>{
        items.value=data.data;
         totalRows.value = data.Count
          perPage.value = 15
      })
      ...
      const searchresults = () =>{
        ApiService.get(`units_status/emergency?page=${currentPage.value}&sortDesc=${sortDesc.value}&template_name=${template_name.value}&client_name=${client_name.value}&unit_number=${unit_number.value}`).then((data)=>
      {
        items.value=data.data;
