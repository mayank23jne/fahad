<template>
    <div class="card">
      ...
      <!--end::Select wrapper-->
            ...
            <button
              ...
              data-bs-toggle="modal"
              data-bs-target="#kt_modal_add_registeredproperty"
            >
              <span class="svg-icon svg-icon-2">
      ...
      <div class="card-body pt-0">
        ...
        >
            ...
            <template v-slot:buildingno="{ row: table }" >
                {{ table.building_no }}
            </template>
            <template v-slot:buildingname="{ row: table }" >
                {{ table.building_name }}
            </template>
            ...
            <template v-slot:licenseno="{ row: table }">
                {{ table.permitno }}
            </template>
           
             <!-- <template v-slot:propertytype="{ row: table }">
            <span v-for="property in propertytype" :key="property.id"
            ><span  v-if="property.id== table.propertytype && lang=='en'||lang == 'enrtl'"> {{ property.name_en }}</span>
            <span  v-if="property.id== table.propertytype && lang == 'ar'"> {{ property.name_ar }}</span> </span>
            </template> -->
      ...
      <RegisteredPropertyAddNewModal :registeredproperty="registeredproperty" :propertytype="propertytype" :units="units" :clientname="clientname" ></RegisteredPropertyAddNewModal>
    ...
    import RegisteredPropertyAddNewModal from "@/containers/masters/RegisteredPropertyAddNewModal.vue";
   ...
   interface units{
    ...
    export default defineComponent({
      ...
      setup() {
        const tableHeader = ref([
          {
            ...
            columnLabel: "buildingno",
            sortEnabled: true,
            columnWidth: 175,
          },
          {
            columnName: "buildingname",
            columnLabel: "buildingname",
            sortEnabled: true,
            columnWidth: 175,
          },
          {
        ...
        const registeredproperty = ref<Array<registeredproperty>>([]);
        const islodaing= ref<Boolean>(true);
        const units=ref<Array<units>>([]);
    ...
    .then(({ data }) => {
      clientname.value = data.data.map((client) => {
        ...
        const statuschange = (status, id)=>{
          ...
          ApiService.post("statusRegisteredProperty", {status:st,
          id:id})
      .then(({ data }) => {  
      setTimeout(() => {
        Swal.fire({
        ...
        const showData=(event)=>{
          ...
          return ApiService.post('archiveRegisteredProperty', {val:val,user_id:user_id})
        .then(({ data }) => {
          registeredproperty.value = data;
        })
        .catch(({ response }) => {
        ...
        const archivechange = (is_deleted, id)=>{
          ...
          ApiService.post("isDeletedRegisteredProperty", {delete:st,
          id:id})
              .then(({ data }) => {  
                registeredproperty.value = registeredproperty.value.filter(e => e.id !== id);
              setTimeout(() => {
                Swal.fire({
                  text: translate("savedsuccess"),
                  icon: "success",
        ...
        const editrow=(id)=>{
          ...
          router.push({name: 'editregisteredproperty', params: { id: id }})
        }
        ...
        const deleterow=(id)=>
    {
          ...
          if (confirmed.value==true) {
            ApiService.delete("deleteRegisteredProperty/"+id)
            .then(({ data }) => {
              registeredproperty.value = registeredproperty.value.filter(e => e.id !== id);
              setTimeout(() => {
              Swal.fire({
