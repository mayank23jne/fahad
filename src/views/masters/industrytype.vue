    ...
    <div class="card">
      ...
      <!--end::Select wrapper-->
            ...
            <button
              ...
              data-bs-toggle="modal"
              data-bs-target="#kt_modal_add_industrytype"
            >
              <span class="svg-icon svg-icon-2">
      ...
      <IndustryTypeAddNewModal :industrytype="industrytype"  ></IndustryTypeAddNewModal>
  ...
  interface industry_type{
    ...
    export default defineComponent({
      ...
      setup() {
        ...
        const industrytype = ref<Array<industry_type>>([]);
      ...
      const { t, te } = useI18n();
          ...
          return ApiService.post('showIndustryType',{user_id:user_id})
        .then(({ data }) => {
        ...
        const statuschange = (status, id)=>{
          ...
          ApiService.post("statusIndustryType", {status:st,
          id:id})
      .then(({ data }) => {  
      setTimeout(() => {
        Swal.fire({
         ...
         const showData=(event)=>{
          ...
          return ApiService.post('archiveIndustryTypes', {val:val,user_id:user_id})
        .then(({ data }) => {
          industrytype.value = data;
        })
        .catch(({ response }) => {
        ...
        const archivechange = (is_deleted, id)=>{
          ...
          if(is_deleted == '0'){ 
            ...
            ApiService.post("isDeletedIndustryType", {delete:st,
            id:id})
        .then(({ data }) => {  
          industrytype.value = industrytype.value.filter(e => e.id !== id);
            setTimeout(() => {
              Swal.fire({
                text: translate("savedsuccess"),
                icon: "success",
        ...
        const editrow=(id)=>{
          ...
          router.push({name: 'editindustrytype', params: { id: id }})
        }
        ...
        const deleterow=(id)=>
    {
          ...
          if (confirmed.value==true) {
            ApiService.delete("deleteIndustryType/"+id)
            .then(({ data }) => {
              industrytype.value = industrytype.value.filter(e => e.id !== id);
              setTimeout(() => {
              Swal.fire({
