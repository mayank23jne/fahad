        ...
        >
          ...
          <div class="card-body border-top p-9">
            ...
            <div class="row mb-6">
              ...
              <label class="col-lg-4 col-form-label required fw-semobold fs-6"
              >{{ translate('selectindustrytype') }}</label
              >
              ...
              <div class="col-lg-8 fv-row">
                ...
                <Field
                  ...
                  v-model="newItem.industrytype"
                    :options="industry_type"
                >
                <template v-for="industry in industry_type" :key="industry.id">
                  <option :value="industry.id" v-if="lang == 'en' || lang == 'enrtl'">{{ industry.name_en }}</option>
                  <option :value="industry.id" v-else>{{ industry.name_ar }}</option>
                </template>
                </Field>
                <div class="fv-plugins-message-container">
                  <div class="fv-help-block">
  ...
  export default defineComponent({
   ...
   props:['id'],
    ...
    const submitButton1 = ref<HTMLElement | null>(null);
     const industry_type=ref<industry_type>();  
      ...
      if (te(text)) {
        ...
        return text;
         ...
         const profileDetailsValidator = Yup.object().shape({
          ...
          owner: Yup.string().required(translate('ownerreq')),
    ...
    onMounted(() => {
         ...
         ApiService.post('showInspectionPropertyId', {id:props.id, user_id:user_id})
        .then(({ data }) => {
          newItem.value.name_ar = data[0].propertyname_ar;
          newItem.value.name_en = data[0].propertyname_en;
          newItem.value.owner = data[0].owner;
          newItem.value.industrytype = data[0].industry_type;
        })
        .catch(({ response }) => {
      ...
      const formSubmit=()=> {
        ...
        data.append("owner", newItem.value.owner);
        data.append("industry_type", newItem.value.industrytype);
        data.append("user_id", user_id);
        data.append('id', props.id);
        ApiService.post("updateInspectionProperty", data)
      .then(({ data }) => {  
      setTimeout(() => {
        Swal.fire({
          ...
          router.push({ name: "inspectionpropertytype" });
        });
      }, 2000);
