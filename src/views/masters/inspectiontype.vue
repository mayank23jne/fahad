<template>
    <div class="card">
      ...
      <!--end::Select wrapper-->
            ...
            <button
              ...
              data-bs-toggle="modal"
              data-bs-target="#kt_modal_add_inspectiontype"
            >
              <span class="svg-icon svg-icon-2">
      ...
      <InspectionTypeAddNewModal :inspectiontype="inspectiontype"  ></InspectionTypeAddNewModal>
    ...
    import InspectionTypeAddNewModal from "@/containers/masters/InspectionTypeAddNewModal.vue";
  ...
  interface inspectiontype{
    ...
    export default defineComponent({
      ...
      setup() {
        ...
        const inspectiontype = ref<Array<inspectiontype>>([]);
      ...
      const { t, te } = useI18n();
          ...
          return ApiService.get('showAllInspectionTypes')
        .then(({ data }) => {
        ...
        const statuschange = (status, id)=>{
          ...
          ApiService.post("statusInspectionType", {status:st,
          id:id})
      .then(({ data }) => {  
      setTimeout(() => {
        Swal.fire({
        ...
        const showData=(event)=>{
          ...
          return ApiService.post('archiveInspectionType', {val:val,user_id:user_id})
        .then(({ data }) => {
          inspectiontype.value = data;
        })
        .catch(({ response }) => {
        ...
        const archivechange = (is_deleted, id)=>{
          ...
          ApiService.post("isDeletedInspectionType", {delete:st,
          id:id})
      .then(({ data }) => {  
        inspectiontype.value = inspectiontype.value.filter(e => e.id !== id);
      setTimeout(() => {
        Swal.fire({
          text: translate("savedsuccess"),
          icon: "success",
        ...
        const editrow=(id)=>{
          ...
          router.push({name: 'editinspectiontype', params: { id: id }})
        }
        ...
        const deleterow=(id)=>
    {
          ...
          if (confirmed.value==true) {
            ApiService.delete("deleteInspectionType/"+id)
            .then(({ data }) => {
              inspectiontype.value = inspectiontype.value.filter(e => e.id !== id);
              setTimeout(() => {
              Swal.fire({
