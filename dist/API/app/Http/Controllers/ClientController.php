...
class ClientController extends Controller
{
    ...
    public function paginate($items, $perPage = 15, $page = null, $options = [])
    {
     ...
     function showAllClientsData(){
       $_data["data"]=array();
       $clients= Admin::where('is_deleted', '0')->orderBy('id', 'desc')->get();
        foreach($clients as $client){
            $data["id"] = $client->id;
            $data["role"] =  Role::where('id', $client->role_id)->first()->role_name;
    ...
    function isDeleteClients(Request $request){
        $clients = Admin::find($request->id);
        $clients->is_deleted = $request->delete;
        $clients->save();
       
        return response()->json("data has been delete successfully");
    }
