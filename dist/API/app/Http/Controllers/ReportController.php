...
class ReportController extends Controller
{
    ...
    function userReportsWheres()
    {
        $_data['users'] = array();
        $_data['Values'] = array();
        $form =  Template::where('inspectiontype','5')->pluck('id');
    ...
    function userReportsHr()
    {
        $_data['users'] = array();
        $_data['Values'] = array();
        $form =  Template::where('inspectiontype','2')->pluck('id');
            $answers = Answer::whereIn('template_id',$form)->select('user_id')
                ->selectRaw('COUNT(*) AS count')
                ->groupBy('user_id')
                ->get();
    ...
    function formWhereAnswerCount()
    {
        ...
        $forms =  Template::where('inspectiontype','5')->get();
    ...
    function formHrAnswerCount()
    {
        $_data['template'] = array();
        $_data['answers_count'] = array();
        $forms =  Template::where('inspectiontype','2')->get();
        foreach($forms as $form){
            $answers = Answer::where('template_id',$form->id)
                ->get();
            array_push($_data['template'], $form->template_name);
    ...
    {
            $data = array();
        $templates = Template::where('inspectiontype','5')->get();
        foreach ($templates as $template) {
    ...
    {
        $data = array();
        $templates = Template::where('inspectiontype','2')->get();
        foreach ($templates as $template) {
    ...
    function sumAnswersForMakkaTemplate()
    {
        $data = array();
        $travels = Answer::where('template_id', 20)->get();
        $travelsCount = Count($travels);

        $Units = $travels->pluck('unit_id');
        $arrUnits = json_decode(json_encode($Units), true);
        $pilCount = AnswerDetail::where('question_id', 855)->sum('answer');
        $workers = AnswerDetail::where('question_id', 845)->sum('answer');
        $time_avg = 2.7;
        $workerAvg = round($workers/$travelsCount,2);
